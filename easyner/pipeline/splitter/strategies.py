...
class ProcessingStrategySelector:
    ...
    def select_strategy(self, batch) -> str:
        ...
        if (
            self.tokenizer.SUPPORTS_BATCH_GENERATOR
            and self.is_pubmed
            and total_articles > 1
        ):
            logger.debug(
            ...
            self.tokenizer.SUPPORTS_BATCH_PROCESSING
            and self.is_pubmed
            and total_articles > 1
        ):
            logger.debug(
...
class BatchGeneratorStrategy(BaseProcessingStrategy):
    ...
    def process(self, processor, batch_idx, batch, text_field, full_articles):
        ...
        texts_to_process, article_ids_in_order, empty_articles, original_order = (
            self.prepare_batch(
                batch,
                text_field,
            )
        ...
        try:
            ...
            sentence_lists_generator = tokenizer.segment_sentences_batch_generator(
                texts_to_process,
            )

            for article_id, sentences in zip(
                article_ids_in_order,
                sentence_lists_generator,
            ):
...
class SingleDocumentStrategy(BaseProcessingStrategy):
    ...
    def process(self, processor, batch_idx, batch, text_field, full_articles):
        ...
        logger.debug(
            f"[Worker {worker_id}] Processing {total_articles} articles individually.",
        )
