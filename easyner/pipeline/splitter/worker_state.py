...
class WorkerStateManager:
    ...
    def start_batch(self, batch_idx, batch):
        ...
        """
        self.current_batch = {"idx": batch_idx, "size": len(batch), "start_time": time.time()}
        logger.debug(
    ...
    def report_error(self, error_message, batch_idx=None):
        """
    ...
    def signal_ready(self, supports_batch, supports_generator):
        """
        ...
        """
        self.result_queue.put(("WORKER_READY", self.worker_id, supports_batch, supports_generator))
        logger.debug(
    ...
    def get_stats(self):
        ...
        if self.stats["articles"] > 0:
            avg_time_per_article = self.stats["processing_time"] / self.stats["articles"]
        else:
