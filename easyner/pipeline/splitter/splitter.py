...
def split_batch(
    splitter_config, batch_idx, batch, full_articles, tokenizer="spacy", model="en_core_web_sm"
):
    """
    Description:
    ...
    with open(
        f'{splitter_config["output_folder"]}/{splitter_config["output_file_prefix"]}_{tokenizer}-split-{batch_idx}.json',
        "w",
        encoding="utf-8",
    ) as f:
